...
$app->get(
  ...
  if (TRUE) {
    ...
    if (!is_null($idAnnonce)) {
      ...
      $annoncesTableSimilar = new \Zend\Db\TableGateway\TableGateway(
        'annonces', $adapter
      );
      $sqlSelectSimilar = $annoncesTableSimilar->getSql()->select();
      $sqlSelectSimilar->limit(6);
      $resultSetSimilar = $annoncesTableSimilar->selectWith($sqlSelectSimilar);
      $similiarAnnonces = utf8ize($resultSetSimilar->toArray());
    }
    else {

    }
    $sqlSelect->join(
      'sites', 'annonces.idSite = sites.idSites',
      array('idSites', 'name', 'logo')
    );
  ...
  $data_return = array(
    ...
    'tags'             => array(),
    'sites'            => array(),
    'similiarAnnonces' => $similiarAnnonces,
  );
